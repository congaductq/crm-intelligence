...
server.resource(
    "system-schema",
    "schema://main",
    { description: "The full MongoDB and Supabase schema documentation" },
    async (uri) => {
...
server.resource(
    "active-stores",
    "stores://all",
    { description: "List of all active stores across the CRM" },
    async (uri) => {
        const matching_logs: string[] = [];
...
async function supabaseCallWithRetry(fn: () => Promise<any>, matching_logs: string[], label: string) {
    const maxAttempts = 2;
...
async function resolveStore(nameOrId: string | undefined, matching_logs: string[]) {
    if (!nameOrId || nameOrId.trim() === "") return { id: null, name: null };
    ...
    if (isObjectId) {
        ...
        const idRes = await supabaseCallWithRetry(() => supabase.from("stores").select("id, store_name").eq("id", nameOrId.trim()) as any, matching_logs, 'Supabase ID lookup');
        const idData = idRes?.data;
        const idError = idRes?.error;
        if (idError && idError.message) matching_logs.push(`Supabase ID lookup error: ${idError.message}`);
    ...
    const d1Res = await supabaseCallWithRetry(() => supabase.from("stores").select("id, store_name").ilike("store_name", `%${fuzzy}%`).limit(1) as any, matching_logs, 'Supabase fuzzy lookup');
    if (d1Res?.error && d1Res.error.message) matching_logs.push(`Supabase fuzzy lookup error: ${d1Res.error.message}`);
    const d1 = d1Res?.data;
    if (d1 && d1.length > 0) {
    ...
    const words = nameOrId.split(/[^a-zA-Z0-9]/).filter(w =>
    ...
    if (words.length > 0) {
        ...
        const tryMatchResult = async (cnt: number) => {
            const segment = words.slice(0, cnt).join("%");
            matching_logs.push(`Tier 2 (Words x${cnt}): ilike %${segment}%`);
            const res = await supabaseCallWithRetry(() => supabase.from("stores").select("id, store_name").ilike("store_name", `%${segment}%`).limit(1) as any, matching_logs, `Supabase tier2 lookup x${cnt}`);
            if (res?.error && res.error.message) matching_logs.push(`Supabase tier2 lookup error: ${res.error.message}`);
            return res?.data;
        };
...
server.tool(
    ...
    async ({ season, month, year, limit, storeName }) => {
        ...
        try {
            ...
            if (resolvedId) sbQuery = sbQuery.eq("store_id", resolvedId);
            ...
            if (month && year) {
                ...
                if (season === "winter") { startDate = new Date(currentYear - 1, 11, 1); endDate = new Date(currentYear, 2, 1); }
                else if (season === "spring") { startDate = new Date(currentYear, 2, 1); endDate = new Date(currentYear, 5, 1); }
                else if (season === "summer") { startDate = new Date(currentYear, 5, 1); endDate = new Date(currentYear, 8, 1); }
                else if (season === "fall") { startDate = new Date(currentYear, 8, 1); endDate = new Date(currentYear, 11, 1); }
            }
            ...
            const sbOrdersRes = await supabaseCallWithRetry(() => sbQuery.limit(1000) as any, matching_logs, 'Supabase orders query');
            ...
            if (resolvedId) {
                const { ObjectId } = await import("mongodb");
                try { matchQuery.store_id = new ObjectId(resolvedId); } catch (e) { matchQuery.store_id = resolvedId; }
            }
            ...
            if (month && year) {
                ...
                else if (season === "spring") months = [3, 4, 5];
                else if (season === "summer") months = [6, 7, 8];
                else if (season === "fall") months = [9, 10, 11];
...
server.tool(
    ...
    async ({ store_id, store_name }) => {
        try {
            ...
            const sbRes = await supabaseCallWithRetry(() => supabase.from("stores").select("*").eq("id", rid).single() as any, matching_logs, `Supabase detail fetch ID ${rid}`);
            ...
            const { ObjectId } = await import("mongodb");
            let mStore;
            try { mStore = await mongoClient.db("bs_int").collection("stores").findOne({ _id: new ObjectId(rid) }); } catch (e) { }
            if (!mStore && rname) mStore = await mongoClient.db("bs_int").collection("stores").findOne({ store_name: rname });
...
server.tool(
    ...
    async ({ city, state, country }) => {
        try {
            ...
            if (city) query = query.filter("location", "cs", JSON.stringify([{ city }]));
            if (state) query = query.filter("location", "cs", JSON.stringify([{ state }]));
            if (country) query = query.filter("location", "cs", JSON.stringify([{ country }]));
...
server.tool(
    ...
    async ({ status, startDate, endDate, storeName }) => {
        ...
        try {
            ...
            if (startDate) sbQuery = sbQuery.gte("date_created", startDate);
            if (endDate) sbQuery = sbQuery.lte("date_created", endDate);

            const sbRes = await supabaseCallWithRetry(() => sbQuery.limit(100) as any, matching_logs, 'Supabase orders analytics');
            ...
            let match: any = {};
            if (resolvedId) try { match.store_id = new ObjectId(resolvedId); } catch (e) { match.store_id = resolvedId; }
            if (status) match.status = status;
            if (startDate || endDate) {
                match.date_created_from_str = {};
                if (startDate) match.date_created_from_str.$gte = new Date(startDate);
                if (endDate) match.date_created_from_str.$lte = new Date(endDate);
            }
...
server.tool(
    ...
    async ({ minCLV, storeName, limit = 10 }) => {
        ...
        try {
            ...
            if (resolvedId) sbQuery = sbQuery.eq("store_id", resolvedId);
            if (minCLV) sbQuery = sbQuery.gte("clv", minCLV);

            const sbRes = await supabaseCallWithRetry(() => sbQuery.order('clv', { ascending: false }).limit(limit) as any, matching_logs, 'Supabase customers query');
            if (sbRes?.data && sbRes.data.length > 0) {
                return {
                    content: [{
                        type: "text",
            ...
            if (minCLV) query.clv = { $gte: minCLV };
...
server.tool(
    ...
    async ({ storeName }) => {
        ...
        try {
            ...
            if (resolvedId) sbQuery = sbQuery.eq("store_id", resolvedId);
            const sbCouponsRes = await supabaseCallWithRetry(() => sbQuery.limit(50) as any, matching_logs, 'Supabase coupons query');
            ...
            if (resolvedId) {
                const { ObjectId } = await import("mongodb");
                try { query.store_id = new ObjectId(resolvedId); } catch (e) { query.store_id = resolvedId; }
            }

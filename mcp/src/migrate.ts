...
async function migrate() {
    try {
        ...
        const storeToLocationsMap = new Map<string, any[]>();
        ...
        while (await storeCursor.hasNext()) {
            ...
            if (sBatch.length === 100) {
                ...
                if (error) console.error("Store batch error:", error.message);
                sCount += sBatch.length;
                if (sCount % 500 === 0) console.log(`Migrated ${sCount} stores...`);
                sBatch = [];
            }
        }
        if (sBatch.length > 0) {
        ...
        while (await productCursor.hasNext()) {
            ...
            if (sid && !migratedStoreIds.has(sid)) continue;
            ...
            if (pBatch.length === 100) {
                ...
                if (pCount % 500 === 0) console.log(`Migrated ${pCount} products...`);
                pBatch = [];
            }
        }
        if (pBatch.length > 0) {
            const { error } = await supabase.from("products").upsert(pBatch);
            if (error) console.error("Final product batch error:", error.message);
            pCount += pBatch.length;
        }
        ...
        while (await orderCursor.hasNext()) {
            ...
            if (oBatch.length === 100) {
                ...
                if (oCount % 500 === 0) console.log(`Migrated ${oCount} orders...`);
                oBatch = [];
            }
        }
        if (oBatch.length > 0) {
            const { error } = await supabase.from("orders").upsert(oBatch);
            if (error) console.error("Final order batch error:", error.message);
            oCount += oBatch.length;
        }
        ...
        while (await customerCursor.hasNext()) {
            ...
            if (cBatch.length === 100) {
                ...
                if (error) console.error("Customer batch error:", error.message);
                cCount += cBatch.length;
                if (cCount % 500 === 0) console.log(`Migrated ${cCount} customers...`);
                cBatch = [];
            }
        }
        if (cBatch.length > 0) {
        ...
        while (await couponCursor.hasNext()) {
            ...
            if (cpBatch.length === 100) {
                ...
                if (error) console.error("Coupon batch error:", error.message);
                cpCount += cpBatch.length;
                if (cpCount % 500 === 0) console.log(`Migrated ${cpCount} coupons...`);
                cpBatch = [];
            }
        }
        if (cpBatch.length > 0) {
